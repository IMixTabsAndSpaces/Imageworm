...
class Editor(QMainWindow):
    ...
    imageDir = os.path.join(os.environ['IW_HOME'], 'worm/images')
    ...
    def initUI(self):               
        ...
        exitAction = QAction(QIcon(Editor.imageDir+'/exit.png'), 'Exit', self)
        exitAction.setShortcut('Ctrl+Q')
        exitAction.setStatusTip('Exit application')
        exitAction.triggered.connect(self.close)
        
        newAction=QAction(QIcon(Editor.imageDir+'/new.png'),'New',self)
        newAction.setShortcut('Ctrl+N')
        newAction.setStatusTip('New Application')
        newAction.triggered.connect(self.__init__)
        
        openAction=QAction(QIcon(Editor.imageDir+'/open.png'),'Open',self)
        openAction.setShortcut('Ctrl+O')
        openAction.setStatusTip('Open Application')
        openAction.triggered.connect(self.openo)
        
        saveAction=QAction(QIcon(Editor.imageDir+'/save.png'),'Save',self)
        saveAction.setShortcut('Ctrl+S')
        saveAction.setStatusTip('Save Application')
        saveAction.triggered.connect(self.save)
        
        undoAction=QAction(QIcon(Editor.imageDir+'/undo.png'),'Undo',self)
        undoAction.setShortcut('Ctrl+Z')
        undoAction.setStatusTip('Undo')
        undoAction.triggered.connect(self.textEdit.undo)
        
        redoAction=QAction(QIcon(Editor.imageDir+'/redo.png'),'Redo',self)
        redoAction.setShortcut('Ctrl+Y')
        redoAction.setStatusTip('Undo')
        redoAction.triggered.connect(self.textEdit.redo)
        
        copyAction=QAction(QIcon(Editor.imageDir+'/copy.png'),'Copy',self)
        copyAction.setShortcut('Ctrl+C')
        copyAction.setStatusTip('Copy')
        copyAction.triggered.connect(self.copy)
        
        pasteAction=QAction(QIcon(Editor.imageDir+'/paste.png'),'Paste',self)
        pasteAction.setShortcut('Ctrl+V')
        pasteAction.setStatusTip('Paste')
        pasteAction.triggered.connect(self.paste)
        
        cutAction=QAction(QIcon(Editor.imageDir+'/cut.png'),'Cut',self)
