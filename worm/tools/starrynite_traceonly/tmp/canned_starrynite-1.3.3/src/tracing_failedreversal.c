			 ...
			 PARTNER_t * const partners)
{
  ...
  if (!nuc_ct || !nuc_ct_prev)
    return;
  if (nuc_ct > NUC_LIMIT || nuc_ct_prev > NUC_LIMIT)
    exit (314);

  for (j = 0; j < NUC_LIMIT; j++)
  {
    partners[j].distance = movie->max_distance;
  }
  ...
  for (i = 0; i < nuc_ct_prev; i++)
  {
    for (j = 0; j < nuc_ct; j++)
    {
      ...
      partners[j].index = j;
      partners[j].distance = distance;
      partners[j].availability = 0;
    }
    qsort (partners, nuc_ct, sizeof (PARTNER_t), compare_distance);
    memcpy (nuclei_prev[i].s_partners, partners, sizeof (PARTNER_t) * PAR_CT);
  }
  ...
  for (j = 0; j < nuc_ct; j++)
  {
    for (i = 0; i < nuc_ct_prev; i++)
    {
      ...
      partners[i].index = i;
      partners[i].distance = distance;
      partners[i].availability = 0;
    }
    qsort (partners, nuc_ct_prev, sizeof (PARTNER_t), compare_distance);
    memcpy (nuclei[j].p_partners, partners, sizeof (PARTNER_t) * PAR_CT);
  }
}
...
SortPartners (const TOOL_t * const tools, const int t)
{
  ...
  for (i = 0; i < nuc_ct_prev; i++)
  {
    ...
    /* qsort (nuclei_prev[i].s_partners, nuc_ct, sizeof (PARTNER_t),
       compare_distance); */  /* ZB, 200908 */
    qsort (nuclei_prev[i].s_partners, PAR_CT, sizeof (PARTNER_t),
           compare_distance);
  }
  ...
  for (i = 0; i < nuc_ct; i++)
  {
    ...
    /* qsort (nuclei[i].p_partners, nuc_ct_prev, sizeof (PARTNER_t),
       compare_distance); */  /* ZB, 200908 */
    qsort (nuclei[i].p_partners, PAR_CT, sizeof (PARTNER_t),
           compare_distance);
  }
}
/* end of modification */
         ...
         const int nuclei_ct)
{
  ...
  /* nuclei_ct is replaced by PAR_CT */
  int countermax;
  countermax=PAR_CT;
  if (countermax>nuclei_ct)	  countermax=nuclei_ct;
            ...
            const int pred, const int index)
{
  ...
  //for (i = 0; i < nuc_ct; i++)
  /* nuc_ct is replaced by PAR_CT */
    int countermax;
  countermax=PAR_CT;
  if (countermax>nuc_ct)	  countermax=nuc_ct;
                          ...
                          const int t, const int index)
{
  ...
  /* nuc_ct_prev is replaced by PAR_CT */
    int countermax;
  countermax=PAR_CT;
  if (countermax>nuc_ct_prev)	  countermax=nuc_ct_prev;
	    for (k = 0; k < countermax; k++)
  //for (k = 0; k < PAR_CT; k++)
  {
...
//	for (k = 0; k < PAR_CT; k++)
	  ...
	  sizeof (PARTNER_t), compare_morphology);*/
        qsort (nuclei_prev[i].s_partners, PAR_CT,
               sizeof (PARTNER_t), compare_morphology);
        //for (k = 0; k < nuc_ct; k++)
	/* nuc_ct is replaced by PAR_CT */
...
static void
AddGhostNucleus (const TOOL_t * const tools, int t, NUCLEUS_t * nuc, int pred,const MOVIE_t * const movie)
{
  NUCLEUS_t *nuclei;
  int nuc_ct;
  ...
  /* ZB 200908 */
  /*
  nuclei[nuc_ct].p_partners = (PARTNER_t *) malloc_exit (sizeof (PARTNER_t) * PAR_CT);
  nuclei[nuc_ct].s_partners = (PARTNER_t *) malloc_exit (sizeof (PARTNER_t) * PAR_CT);
        int j;
      PARTNER_t *const partners1 = nuclei[nuc_ct].p_partners;
      PARTNER_t *const partners2 = nuclei[nuc_ct].s_partners;
      for (j = 0; j < PAR_CT; j++)
      {
      ...
      if (!nuclei_prev[i].ghost)
      {
        if (!nuclei_prev[i].ghost_proof)
        {
          fprintf (logfile,
                   ...
                   i + 1);
          AddGhostNucleus (tools, t, nuclei_prev + i, i,movie);
          err_ct++;
        }
      }
...
static void
ResetNuclei (const MOVIE_t * const movie, const TOOL_t * const tools, const int t)
{
  int i;
...
Trace (const MOVIE_t * const movie, PARAMETER_t * const parameters,
       const TOOL_t * const tools, FILE * const logfile,  int round)
{
  int t, tp_number;
  ...
  /* pre-allocation only needed once hence out of the while loop below */
  /*
   for (t = 0; t < tp_number; t++)
  {
   ...
   fflush (logfile);
    ...
    {
      NUCLEUS_t *const nuc = nuclei + i;
      nuc->p_partners = (PARTNER_t *) malloc_exit (sizeof (PARTNER_t) * PAR_CT);
      nuc->s_partners = (PARTNER_t *) malloc_exit (sizeof (PARTNER_t) * PAR_CT);
      /* the allocation above provides recording of PAR_CT nearest neighbors, instead of all nuclei */
      /* all counters of partners will be changed from nuc_ct to PAR_CT */
/*
      int j;
      PARTNER_t *const partners1 = nuc->p_partners;
      PARTNER_t *const partners2 = nuc->s_partners;
  ...
  /* a common holder of distance to all nuclei is provided and passed on to MeasureNuclearDistances */
  PARTNER_t *partners;
  partners = (PARTNER_t *) malloc_exit (sizeof (PARTNER_t) * NUC_LIMIT);
  ...
  /* ZB, 200908, to replace the recursive call with while loop */
  int err_ct = 1;
  while (err_ct)
  {
    ...
    for (t = 1; t < tp_number; t++)
    {
      ...
      MeasureNuclearDistances (movie, parameters, tools, logfile, t, partners);
      //SortPartners (tools, t); /* ZB, 200908, since the partners are already sorted in the current version of MeasureNuclear Distances before they're stored. */
    ...
    {
      for (t = 0; t < tp_number; t++)
        ResetNuclei (movie, tools, t);
      /* Trace (movie, parameters, tools, logfile, round + 1); */
      /* to replace the round counter in the previous recurssive call */
      round ++;
    }
  }
}
