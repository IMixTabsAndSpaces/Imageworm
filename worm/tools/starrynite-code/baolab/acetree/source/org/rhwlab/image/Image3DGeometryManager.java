 ...
 * of Image3D2
 *
 * creates nuclear scenegraph to return to Image3D2
 *
 * Formerly Nuclei3D (Inner class of Image3D2) 
 * 2.21.2014
 */
...
public class Image3DGeometryManager {
    ...
    public Image3DGeometryManager(Image3D2 parent) {
    ...
    public BranchGroup createNuclearBranchGroup(int iXA, int iYA, float iZA) {
        ...
        if(viewConfig.isShowingOverlay() && overlayReady) {
            addOverlay();
        }
    ...
    */
    ...
    private Nucleus addOne(String name, boolean sister) {
        ...
        iShowIt = true;
        iTransparent = false;
        if (k < viewConfig.getNumDispProps() - 2) app = getLineageColor(k);
...
TransparencyAttributes semitran = new TransparencyAttributes(TransparencyAttributes.BLENDED, 0.25f);
            ...
            int k = getLineageNumber(n.identity);
            int rule = getLineageRule(n.identity);//these were assumed same, has to be a bug
            iShowIt = true;
            if (rule < viewConfig.getNumDispProps() - 2){ 
                app = getLineageColor(k);
                //app.setTransparencyAttributes(semitran);
            }
            else {
                int m = viewConfig.getDispProp(viewConfig.getNumDispProps() - 2).getLineageNum();
                switch(m) {
	...
	String propername=PartsList.lookupSulston(name);
        ...
        for (i = 0; i < viewConfig.getNumDispProps(); i++) {
	    boolean matchpropername=false;
	    matchpropername=propername!=null&&propername.equals(viewConfig.getDispProp(i).getName());
            if (matchpropername|name.indexOf(viewConfig.getDispProp(i).getName()) >= 0) {
                num = viewConfig.getDispProp(i).getLineageNum();
                break;
            }
