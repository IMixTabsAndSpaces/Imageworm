...
public class VTree implements ActionListener {
	...
	public VTree() {
		...
		iFrame.setDefaultLookAndFeelDecorated(false);
	...
	private JScrollPane makePanel() {
		...
		/*
		 * iPng = new JRadioButton("png file"); iPs = new JRadioButton("postscript");
		 * ButtonGroup bg = new ButtonGroup(); bg.add(iPng); bg.add(iPs);
		 * iPs.setSelected(true); JPanel jp = new JPanel(new GridLayout(1,0)); Border
		 * blackline = BorderFactory.createLineBorder(Color.black);
		 * jp.setBorder(blackline); jp.add(iPng); jp.add(iPs); pWhole.add(jp);
		 */
		JScrollPane sPane = new JScrollPane(pWhole);
		return sPane;
	}

	private String[] labels = { "rootCell", "endTime", "minRed", "maxRed", "yInc", "lineWidth", "hue"
			// ,"fileName"
			// ,"pagesBreaks"
	};

	private String[] initValues = { "E", "200", "-500", "5000", "10", "5", "0"
			// ,"VTree.ps"
			// ,"NONE"
	};
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o == iShow) {
			...
			Color c = JColorChooser.showDialog(null, "Choose expression color", Color.RED);
			if (c != null) {
