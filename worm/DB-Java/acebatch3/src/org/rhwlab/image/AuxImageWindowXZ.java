...
public class AuxImageWindowXZ extends JFrame {
	...
	public AuxImageWindowXZ(String title, ImageProcessor dataProc, AceTree aceTree, int y) {
		super(title);
		iTitle = title;
		iAceTree = aceTree;
		iYLoc = y;
	...
	protected void updateImagePlus() {
		...
		iproc3.setRGB(R, G, B);
		if (iShowCentroids)
			showCentroids(iTime, iYLoc, iproc3);
		if (iShowAnnotations)
			showAnnotations(iproc3);
	...
	class MouseHandler extends MouseInputAdapter {
		...
		public void mouseClicked(MouseEvent e) {
			...
			if (iAnnotsShown.get(ai.iName) != null)
				iAnnotsShown.remove(ai.iName);
			else
				iAnnotsShown.put(ai.iName, ai);
			updateImagePlus();
		}
	...
	private static final String CS = ", ", POSITION = "xz = ";
