...
public class ExtractRed extends Log {
	...
	private void processAllCircles(Vector allCircles, Vector circles, boolean report) {
		...
		while (ee.hasMoreElements()) {
			...
			if (report)
				append(cc.index + C.CS + redCount + C.CS + redWeight + C.CS + cc.n.rweight + C.CS + redSum + C.CS
						+ cc.weight + C.CS + cc.name);
		}
	...
	private ImageProcessor getRedData(String greenName) {
		...
		if (ImageWindow.cUseZip == 2)
			return getRedZipData(greenName);
		FileInputStream fis;
		ImagePlus ip = null;
	...
	private class Centroid implements Comparator {
		...
		public int compare(Object o1, Object o2) {
			if (((Centroid) o1).plane < ((Centroid) o2).plane)
				return -1;
			else if (((Centroid) o1).plane > ((Centroid) o2).plane)
				return 1;
			return 0;
		}
	}
	...
	private static final String CLEAR = "Clear", TEST1 = "One", TEST2 = "All", TEST3 = "Test3";

	public static void main(String[] args) {
	}

	/*
	 * private void processAllCirclesJAI(Vector allCircles, Vector circles, boolean
	 * report) { Collections.sort(allCircles, new Centroid()); Enumeration e =
	 * allCircles.elements(); int plane = 0; PlanarImage ipRed = null; while
	 * (e.hasMoreElements()) { Centroid c = (Centroid)e.nextElement(); if (c.plane >
	 * plane) { plane = c.plane; String imageFile = ImageWindow.cZipTifFilePath;
	 * imageFile += "/" + ImageWindow.cTifPrefixR; imageFile +=
	 * iAceTree.makeImageName(c.time, c.plane); ipRed = getRedZipDataJAI(imageFile);
	 * System.out.println("XimageFile: " + (new File(imageFile).getName()));
	 * 
	 * } int [] ia = processImageJAI(ipRed, c); c.rsum = ia[0]; c.rcount = ia[1];
	 * System.out.println("X: " + c.name + C.CS + c.rsum + C.CS + c.rcount); } //
	 * each element of circles has data for one cell // the data consists of several
	 * planes where the cell is observable // in each plane we need to count the red
	 * stuff // and accumulate it against the cell involved Enumeration ee =
	 * circles.elements(); while (ee.hasMoreElements()) { Vector vv =
	 * (Vector)ee.nextElement(); // vv is deals with one of the cells present at
	 * this time point // it may have several Centroids in it long redSum = 0; long
	 * redCount = 0; Centroid cc = null; Enumeration ec = vv.elements();
	 * while(ec.hasMoreElements()) { cc = (Centroid)ec.nextElement(); redSum +=
	 * cc.rsum; redCount += cc.rcount; System.out.println("X: " + cc.name + C.CS +
	 * redSum + C.CS + redCount); } int redWeight = (int)(redSum * STANDARDCOUNT /
	 * redCount); cc.n.rweight = redWeight; if (report) append(cc.index + C.CS +
	 * redCount + C.CS + redWeight + C.CS + cc.n.rweight + C.CS + redSum + C.CS +
	 * cc.weight + C.CS + cc.name); }
	 * 
	 * }
	 * 
	 * private PlanarImage getRedZipDataJAI(String redName) { return
	 * getPlanarImage(redName); }
	 * 
	 * PlanarImage getPlanarImage(String zipfilename) { PlanarImage source = null;
	 * File f = new File(zipfilename);
	 * 
	 * if ( f.exists() && f.canRead() ) { String s = f.getName(); int k =
	 * s.indexOf("."); s = s.substring(0, k + 1); s += "tif"; try { ZipFile zf = new
	 * ZipFile(f); ZipEntry ze = zf.getEntry(s); InputStream is =
	 * zf.getInputStream(ze); SeekableStream stream =
	 * SeekableStream.wrapInputStream(is, true); source = JAI.create("stream",
	 * stream); } catch(Exception ioe) { ioe.printStackTrace(); }
	 * 
	 * } return source; }
	 * 
	 * public int [] processImageJAI(PlanarImage ipRed, Centroid c) { //int d =
	 * (int)(c.d + 0.5); int x = c.x; int y = c.y; double dia = c.d; double area =
	 * Math.PI * dia * dia; double side = Math.sqrt(area); int intSide =
	 * (int)Math.round(side); x -= side/2; y -= side/2; double mean = getMean(ipRed,
	 * x, y, intSide, intSide);
	 * 
	 * int [] ia = new int[2]; ia[0] = (int)Math.round(mean * area); ia[1] =
	 * (int)Math.round(area);; return ia; }
	 * 
	 * private double getMean(PlanarImage img, int x, int y, int width, int height)
	 * { ParameterBlock pb = new ParameterBlock(); pb.addSource(img); // The source
	 * image pb.add(new ROIShape(new Rectangle(x, y, width, height))); pb.add(1); //
	 * check every pixel horizontally pb.add(1); // check every pixel vertically
	 * RenderedImage meanImage = JAI.create("mean", pb, null); double[] mean =
	 * (double[])meanImage.getProperty("mean"); return mean[0];
	 * 
	 * }
	 */
