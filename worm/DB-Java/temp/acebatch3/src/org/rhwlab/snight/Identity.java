...
/**
 * Singleton object to set cell names in nuclei files Based originally on the
 * StarryNite cell_identity.h code
 * 
 * @author biowolp
 *
 */
public class Identity {
	...
	public Identity(NucleiMgr nucleiMgr) {
		...
		// iCanonicalSimpleNamesHash =
		// CanonicalTree.getCanonicalTree().getCanonicalSimpleNamesHash();
	...
	private double getAngle() {
		...
		if (x.length() > 0)
			iXCenter = Double.parseDouble(x);
		if (y.length() > 0)
			iYCenter = Double.parseDouble(y);
	...
	public void useCanonicalRules(int[] start, int[] lineage_ct_p) {
		...
		for (i = start[0]; i < m; i++) {
			...
			if (rotate_axis > 0 && nuc_ct > Identity.EARLY) {
				rotateAxis();
				println("useCanonicalRules: after rotateAxis, " + iParameters.ap + CS + iParameters.dv + CS
						+ iParameters.lr);
...
//        rtn = rtn && !pname.equals("Z3");
	...
	private void nameDaughters(Nucleus parent, Nucleus dau1, Nucleus dau2) {
		tag1 = iTag;
		if (specialCases(parent, dau1, dau2))
			return;
		dau1.identity = parent.identity + iTag;
		dau2.identity = replaceLastChar(dau1.identity);
	}

	private static final int XCENTER = 350, YCENTER = 250;
	...
	public void identityAssignment() {
		System.out.println("\n\nidentityAssignment: " + iParameters.axis + C.CS + iParameters.ap + C.CS + iParameters.dv
				+ C.CS + iParameters.lr);
		iMovie = iNucleiMgr.getMovie();
		iNucCount = 1;
		...
		for (int i = start[0]; i < iEndingIndex; i++) {
			...
			for (int j = 0; j < nuc_ct; j++) {
				...
				if (nucleij.status == DEAD || nucleij.status == DEADZERO)
					continue;
	...
	public void newSisterID(Nucleus parent, Nucleus nuc1, Nucleus nuc2, int cellCount) {
		...
		if (cellCount < EARLY)
			tag1 = earlyFirstCellTag(k);
		else
			tag1 = midFirstCellTag(k);
	...
	/**
	 * return -1 if x is controlling and cd1 is left of cd2 return +1 if x is
	 * controlling and cd1 is right of cd2 return -2 if y is controlling and cd1 is
	 * above cd2 return +2 if y is controlling and cd1 is below cd2 return -3 if z
	 * is controlling and cd1 is in front of cd2 return +3 if z is controlling and
	 * cd1 is behind cd2 decide on control using the existing StarryNite rules
	 *
	 */
	...
	private int fourCellID(int four_cells, int[] lineage_ct_p) {
		...
		for (i = 0; i < nuc_ct; i++) {
			...
			if (nucleii.identity.indexOf(POLAR) > -1)
				continue;
			// 20050809 changed sense of the next line -- should now match SN
			if (nucleii.predecessor == Nucleus.NILLI)
				lin_ct++;
	...
	private int timeToDivide(int current_time, Nucleus nuc) {
		while (current_time < iEndingIndex) {
			if (nuc.successor1 == Nucleus.NILLI)
				return -1;
	...
	private void polarBodies() {
		...
		for (i = 0; i < iEndingIndex; i++) {
			...
			for (int j = 0; j < nuc_ct; j++) {
				...
				if (nucleij.identity.indexOf(POLAR) == -1)
					continue;
				if (nucleij.successor1 == Nucleus.NILLI)
					p_ct--;
				if (p_ct == 0)
					break;
				if (nucleij.successor2 != Nucleus.NILLI) {
					System.out.println("Polar body divided: " + i + 1 + ":" + j + 1 + "->" + i + 2 + ":"
							+ nucleij.successor1 + " and " + i + 2 + ":" + nucleij.successor2);
				} else {
	...
	private int countCells(Vector nuclei) {
		...
		for (int i = 0; i < nuclei.size(); i++) {
			...
			if (n.status > -1 && n.identity.indexOf(POLAR) == -1)
				cell_ct++;
		}
		return cell_ct;
	}
	...
	private void clearAllNames() {
		...
		for (int i = iStartingIndex - 1; i < iEndingIndex; i++) {
			...
			if (!(i < k))
				break;
			// for (int i = 0; i < iEndingIndex - iNucleiMgr.getConfig().iStartingIndex;
			// i++) {
			...
			for (j = 0; j < nuc_ct; j++) {
				...
				if (!validId && nucleij.predecessor == Nucleus.NILLI) {
					...
					if (successor2 == Nucleus.NILLI) {
						...
						newBornID(pred, (Nucleus) nuclei.elementAt(pred.successor1 - 1),
...
//                if (nucleij.successor2 != Nucleus.NILLI) {
				if (((Nucleus) nuclei.elementAt(j)).successor2 != Nucleus.NILLI) {
					...
					sisterID((Nucleus) nuclei_next.elementAt(nucleij.successor1 - 1),
							(Nucleus) nuclei_next.elementAt(nucleij.successor2 - 1), nuc_ct);
				}
			}
		}
		lineage_ct_p[0] = lin_ct;
	...
	private static final char E = 'e', W = 'w', V = 'v', D = 'd', B = 'b' // 'd' based on identity study 20050614
			, T = 't' // 'v'
			, A = 'a', P = 'p', L = 'l', R = 'r', X = 'X' // a dummy tag used in newBornID
			, N = 'n', S = 's', IGNORESULSTON = 'i';
...
/*
 * private String getOrientation() { String orientation = "A"; String late =
 * "A"; if (iParameters.ap < 0) orientation = "P"; if (iParameters.dv > 0)
 * orientation += "D"; else orientation += "V"; if (iParameters.lr > 0)
 * orientation += "L"; else orientation += "R"; if (orientation.equals("ADL"))
 * late = "ARD"; else if (orientation.equals("AVR")) late = "ALV"; else if
 * (orientation.equals("PDR")) late = "PLD"; else if (orientation.equals("PVL"))
 * late = "PRV"; return orientation + " -> " + late; }
 */
