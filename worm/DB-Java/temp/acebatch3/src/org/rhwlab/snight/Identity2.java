...
public class Identity2 {
	...
	public void useCanonicalRules(int[] start, int[] lineage_ct_p) {
		...
		for (i = start[0]; i < m; i++) {
			...
			if (rotate_axis > 0 && nuc_ct > Identity.EARLY) {
				// rotateAxis();
				// println("useCanonicalRules: after rotateAxis, " + iParameters.ap + CS +
				// iParameters.dv + CS + iParameters.lr);
	...
	public void identityAssignment() {
		System.out.println("\n\nidentityAssignment: " + iParameters.axis + CS + iParameters.ap + CS + iParameters.dv
				+ CS + iParameters.lr);
		iMovie = iNucleiMgr.getMovie();
		iNucCount = 1;
					...
					if (nucleij.predecessor == Nucleus.NILLI || pred.status == Nucleus.NILLI) {
						...
						if (pred.successor2 == Nucleus.NILLI) {
							...
							if (r != 0) {
								...
								/*
								 * if (iNamingMethod != STANDARD) { sisterID(dau1, dau2, nuc_ct); dau1.identity
								 * = pred.identity + dau1.id_tag; dau2.identity = pred.identity + dau2.id_tag; }
								 * else { newSisterID(pred, dau1, dau2, nuc_ct); }
								 */
							}
