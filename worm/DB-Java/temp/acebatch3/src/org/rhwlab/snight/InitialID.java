...
public class InitialID {
	...
	private int alignDiamond(Vector nuclei) {
		...
		for (i = 0; i < nuclei.size(); i++) {
			...
			ia[0] = nucleii.x;
			ia[1] = nucleii.y;
			// println("alignDiamond, BEFORE, " + ia[0] + CS + ia[1] + CS + xmin + CS + xmax
			// + CS + ymin + CS + ymax);
...
//                if (nucleij.successor2 != Nucleus.NILLI) {
	...
	public int newBornID(Nucleus mother, Nucleus dau1, Nucleus dau2) {
		// println("newBornID: " + mother.identity + CS + dau1.identity + CS +
		// dau2.identity);
	...
	private static final String NUC = "Nuc", POLAR = "polar";

	public static final int DIVISOR = 8, MINCUTOFF = 5, XCENTER = 350, YCENTER = 250, EARLY = 50, MID = 450, DEAD = -1,
			DEADZERO = 0;

	private static final char IGNORESULSTON = 'i', A = 'a', D = 'd', L = 'l', V = 'v', E = 'e', W = 'w', N = 'n',
			S = 's', B = 'b' // 'd' based on identity study 20050614
			, T = 't' // 'v'
			, P = 'p', R = 'r', X = 'X' // a dummy tag used in newBornID
