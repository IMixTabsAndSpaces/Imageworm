...
public class Identity3 {
	...
	public void useCanonicalRules(int[] start, int[] lineage_ct_p) {
		...
		// println("Identity3.useCanonicalRules, series = " + series + ", axis = " +
		// iAxis);
		...
		// System.out.println("useCanonicalRules starting at: " + start[0] + CS +
		// iEndingIndex);
		int nuc_ct = 0;
		int i;
		Vector nuclei = null;
		...
		for (i = start[0]; i < m; i++) {
			...
			for (int j = 0; j < nuc_ct; j++) {
				...
				// println("useCanonicalRules, " + i + CS + j + CS + parent.identity + CS +
				// parent.status);
				if (parent.status == Nucleus.NILLI) {
					// println("useCanonicalRules, XXX, " + i + CS + j + CS + parent.identity + CS +
					// parent.status);
					continue;
				}
				...
				if (!good) {
					...
					if (parent.successor1 > 0) {
						...
						if (n.assignedID.length() <= 0) {
							// println("useCanonicalRules, XXXXXX, " + i + CS + j + CS + parent.identity +
							// CS + parent.status + CS + n.identity);
							n.identity = pname;
						} else {
							int kkk = 0;
						}
	...
	public void setNamingMethod(int method) {
		// System.out.println("Identity.setNamingMethod called with: " + method + CS +
