...
public class DivisionCaller {
	...
	StringBuffer assignNames(Nucleus parent, Nucleus dau1, Nucleus dau2, boolean obsolete) {
		...
		/*
		 * if (orientation.equals("ADL")) late = "ARD"; else if
		 * (orientation.equals("AVR")) late = "ALV"; else if (orientation.equals("PDR"))
		 * late = "PLD"; else if (orientation.equals("PVL")) late = "PRV";
		 */
		...
		/*
		 * boolean nochange1 = oldd1.equals(newd1); boolean nochange2 =
		 * oldd2.equals(newd2); boolean changed = !(nochange1 && nochange2); //changed =
		 * changed && (dotCorrected * dotStraight < 0);
		 * 
		 * if (changed) { sb = new StringBuffer(r.toString()); sb.append(C + iAxis + C +
		 * iAxisUse); sb.append(C + fmt4(dotCorrected)); //sb.append(C +
		 * fmt4(dotStraight)); sb.append(C + fmt4(daCorrected[0]) + C +
		 * fmt4(daCorrected[1]) + C + fmt4(daCorrected[2])); //sb.append(C +
		 * fmt4(daStraight[0]) + C + fmt4(daStraight[1]) + C + fmt4(daStraight[2]));
		 * 
		 * }
		 */
	...
	double[] diffsStraight(Nucleus d1, Nucleus d2) {
		...
		da[2] = d2.z - d1.z;
		da[2] *= iZPixRes;
		// if (iDebug) println("diffs, " + fmt4(da[0]) + CS + fmt4(da[1]) + CS +
		// fmt4(da[2]));
		// measurementCorrection(da);
		// if (iDebug) println("diffs, " + fmt4(da[0]) + CS + fmt4(da[1]) + CS +
		// fmt4(da[2]));
