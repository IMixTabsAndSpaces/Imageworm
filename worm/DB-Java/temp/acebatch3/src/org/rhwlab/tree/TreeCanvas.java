...
class TreeCanvas extends Canvas {
	...
	private void displayHash() {
		...
		while (eKeys.hasMoreElements()) {
			...
			System.out.println("displayHash: " + cname + CS + x + CS + cname.iEndingIndex + CS + cname.getTime() + CS
					+ cname.getEndTime());
		}
	}
	...
	private static final int XSCALE = 20, YSCALE = 70, YFIXEDSCALE = 1000, MINWIDTH = 400;
	...
	private Cell findIt(int x, int y) {
		...
		// System.out.println("findIt: " + c + CS + time + CS + Cell.START1 + CS +
		// c.yStartUse);
		while (ev.hasMoreElements()) {
			...
			if (time < c.getTime() || time > c.getEndTime())
				continue;
