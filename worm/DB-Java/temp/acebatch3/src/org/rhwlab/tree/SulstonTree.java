...
public class SulstonTree extends JFrame implements ActionListener, WindowFocusListener {
	...
	public SulstonTree(Object tree, String title, Cell c, boolean canInterrogate) {
		...
		setResizable(true);
		if (canInterrogate)
			iAceTree = (AceTree) tree;
		else
			iCanonicalTree = (CanonicalTree) tree;
		iCurrentCell = c;
		...
		JScrollPane sp = new JScrollPane(tp, JScrollPane.VERTICAL_SCROLLBAR_NEVER,
	...
	public void saveImage() {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			/*
			 * Rectangle screenRect = this.getBounds(); int topAdjust = 58; int y =
			 * screenRect.y; screenRect.y += topAdjust; int height = screenRect.height;
			 * screenRect.height -= topAdjust; // create screen shot Robot robot = null; try
			 * { robot = new Robot(); BufferedImage image =
			 * robot.createScreenCapture(screenRect); ImageIO.write(image, IMAGETYPE, new
			 * File(iTitle + "." + IMAGETYPE)); } catch(AWTException awtex) {
			 * awtex.printStackTrace(); } catch(Exception e) { e.printStackTrace(); }
	...
	private static final String CS = ", ", IMAGETYPE = "png";

	private static final int WIDTH = 800, HEIGHT = 300, MINWIDTH = 400, MINHEIGHT = 100;;
