...
public class Siamese extends JDialog implements ActionListener, ListSelectionListener, Comparator {
	...
	public Siamese() {
		...
		/*
		 * setPreferredSize(new Dimension(700, 300)); setLayout(new BoxLayout(this,
		 * BoxLayout.Y_AXIS)); iToolBar = new JToolBar(""); iToolBar.setLayout(new
		 * GridLayout(1,0)); add(iToolBar); buildOutToolBar(); buildList();
		 * initialize(); iFrame = new JFrame(iTitle); showMe(); //help();
		 */
	...
	public void showDivisions(int now) {
		...
		for (int j = 0; j < nucleiNow.size(); j++) {
			...
			if (nnow.successor1 > 0 && nnow.successor2 > 0) {
				...
				iListModel
						.addElement(division(parentN.identity, a.identity, p.identity, endTime, score) + CS + "PARENT");
	...
	private static int MINLIFETIME = 15, MAXCGMOVEMENT = 20, MAXMOVEMENT = 30, MINMOVEMENT = 10;
	...
	private int evaluateDivision(Nucleus parent, Nucleus dau1, Nucleus dau2, int now) {
		...
		// if (!good) println("bogus, " + parent.identity + CS + now + CS + ddau1 + CS +
		// ddau2 + CS + lifeTime);
		// println("evaluateDivision, " + parent.identity + CS + now + CS + lifeTime +
		// CS + cgdist + CS + ddau1 + CS + ddau2 + CS + score);
		return score;
	}
	...
	String[] shelp = { "# maxTime field accepts forms like 200:ABalaa",
			"# if the colon and cell are there then output will be filtered to that", "# and its decendents",
	...
	private static final String CLEAR = "Clear", LINE = "                                        ", ANGLE = "Angle",
			TEST1 = "Deaths", TEST2 = "Adj back", TEST3 = "Adj forward", TEST4 = "Jumps", TEST5 = "Test5",
			HELP = "Help";
