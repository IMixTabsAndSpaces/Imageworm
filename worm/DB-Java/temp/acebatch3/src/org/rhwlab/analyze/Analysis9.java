...
/**
 * Tools for displaying red expression and making it available as a tab
 * delimited file for use in external programs.
 *
 * @author biowolp
	...
	private void test1() {
		...
		for (int j = 0; j < nuclei.size(); j++) {
			...
			if (n.status == Nucleus.NILLI)
				continue;
			if (n.identity.indexOf(founder) != 0)
				continue;
...
/*
 * private void test3() { int q = 0; long start = System.currentTimeMillis();
 * Vector circles = new Vector(); int k =
 * Integer.parseInt(iTextField2.getText()); Vector nuclei =
 * (Vector)nuclei_record.elementAt(k - 1); Collections.sort(nuclei, this);
 * Nucleus n; String founder = iTextField1.getText(); boolean headerPrinted =
 * false; //appendx(founder); //for (int i=k; i >0; i--) appendx(C.TAB + i);
 * //append(""); //System.out.println("checked: " + iCheckBox.isSelected()); int
 * [] count = new int[1]; int total = 0; for (int j=0; j < nuclei.size(); j++) {
 * n = (Nucleus)nuclei.elementAt(j); if (n.status == Nucleus.NILLI) continue; if
 * (n.identity.indexOf(founder) != 0) continue; String s = n.identity;
 * //appendx(s); Vector strings = new Vector(); Vector headers = new Vector();
 * //strings.add(s); //boolean cellNameAdded = false; for (int
 * m=founder.length(); m <= n.identity.length(); m++) { String cname =
 * n.identity.substring(0,m); //System.out.println("inquiry: " + cname); Cell c
 * = (Cell)iCellsByName.get(cname); //String ss = c.getReverseRedDataString(0,
 * k, 1, count); String ss = c.getRedDataString(0, k, 1, count); //if
 * (!cellNameAdded) { // cellNameAdded = true; // ss = n.identity + ss; //}
 * total += count[0]; strings.add(ss); if (iCheckBox.isSelected())
 * headers.add(0, c.getReverseRedDataHeaderString()); } if (!headerPrinted) {
 * headerPrinted = true; System.out.println("total: " + total + C.CS + k);
 * appendx(founder); //for (int i=k; i > (k-total); i--) appendx(C.TAB + i); for
 * (int i=k - total + 1; i <= k; i++) appendx(C.TAB + i); append("");
 * 
 * } //Enumeration e = strings.elements(); for (int i=0; i < strings.size();
 * i++) { String s2 = (String)strings.elementAt(i); if (i == 0) s2 = n.identity
 * + s2; appendx(s2); } append(""); if (iCheckBox.isSelected()) {
 * appendx(n.identity); Enumeration e = headers.elements(); while
 * (e.hasMoreElements()) { String s2 = (String)e.nextElement(); appendx(s2); }
 * 
 * append(""); } //System.out.println("q=" + q); //iText.setFont(iFont[q]);
 * //append(s);
 * 
 * }
 * 
