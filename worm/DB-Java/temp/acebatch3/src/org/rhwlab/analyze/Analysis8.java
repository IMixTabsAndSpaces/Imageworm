...
/**
 * Consider this a template for future analyses. Obsolete right now and not
 * called.
 *
 * @author biowolp
...
public class Analysis8 extends Log {
	...
	public void test1() {
		...
		/*
		 * String imageFileBase = ImageWindow.cZipTifFilePath; imageFileBase += "/" +
		 * ImageWindow.cTifPrefix; //imageFile += "/" + ImageWindow.cTifPrefixR; String
		 * imageFile = imageFileBase + iAceTree.makeImageName(k, 1); append(imageFile);
		 * ImageProcessor ip = getRedData(imageFile); int width = ip.getWidth(); int
		 * height = ip.getHeight(); ImagePlus iplus = null; iStack = null; for (int
		 * plane = 1; plane <= iNucleiMgr.getPlaneEnd(); plane++) { imageFile =
		 * imageFileBase + iAceTree.makeImageName(k, plane); ImageProcessor iproc =
		 * getRedData(imageFile); if (iplus == null) { iplus = new
		 * ImagePlus(String.valueOf(plane), iproc); iStack = iplus.getStack(); } else {
		...
		/*
		 * System.out.println("ip=" + ip); ImagePlus iplus = new ImagePlus("test", ip);
		 * ij.gui.ImageWindow iwin = new ij.gui.ImageWindow(iplus); processImage(iplus);
		 * //iplus.getProcessor().resetRoi(); iplus.setRoi((Roi)null);
		 * iplus.getProcessor().add(-100); iwin.updateImage(iplus);
		 */
	}
