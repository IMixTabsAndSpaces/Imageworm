...
public class Analysis10 extends Log {
	...
	private void test2() {
		...
		// ByteProcessor bp = new ByteProcessor(ImageWindow.cImageWidth,
		// ImageWindow.cImageHeight);
	...
	protected void showCentroids(int time, int x, ImageProcessor iproc) {
		...
		while (e.hasMoreElements()) {
			...
			if (u > 0) {
				...
				// if (n.hashKey != null &&
				// n.hashKey.equals(iAceTree.getCurrentCell().getHashKey()) &&
				// iAceTree.isTracking()) {
				// iproc.setColor(COLOR[iDispProps[SCENTROID].iLineageNum]);
				// iproc.setColor(Color.white);
				// System.out.println("showAnnotations dia, iImagePlane, iPlaneInc: " + x + CS +
