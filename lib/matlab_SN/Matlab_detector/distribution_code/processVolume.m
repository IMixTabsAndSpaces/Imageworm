...
if (nodatause||nodata)
    ...
    if(isstruct(previous))
        if(~isempty(previous.diams)) %update only if found cells last time otherwise dont do anything
            celldiameter=mean(previous.diams);
        end
    else
...
%disk representation
nucleiSet=struct;
[diskSet,nucleiSet.centerindicies]=createDiskSet(X,maximathreshold,zlevel,celldiameter,anisotropy,numcells);
