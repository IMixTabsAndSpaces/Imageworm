...
public class ImageWindowNoUI {
	...
	protected void showSpecialEffect() {
		/*
		 * if (!iAceTree.isTracking()) return; int x1 =
		 * ((Integer)iSpecialEffect[0]).intValue(); int y1 =
		 * ((Integer)iSpecialEffect[1]).intValue(); int z1 =
		 * ((Integer)iSpecialEffect[2]).intValue(); int x2 =
		 * ((Integer)iSpecialEffect[3]).intValue(); int y2 =
		 * ((Integer)iSpecialEffect[4]).intValue(); int r2 =
		 * ((Integer)iSpecialEffect[5]).intValue(); int z2 =
		 * ((Integer)iSpecialEffect[6]).intValue(); String s =
		 * (String)iSpecialEffect[7]; int offset = r2 + 4; if (y2 < y1) offset =
		 * -offset;
		 * 
		 * 
		 * ImageProcessor iproc = getImagePlus().getProcessor();
		 * //iproc.setColor(Color.magenta);
		 * iproc.setColor(COLOR[iDispProps[LOWERSIS].iLineageNum]); if (z2 <= z1)
		 * iproc.setColor(COLOR[iDispProps[UPPERSIS].iLineageNum]); //if (z2 <= z1)
		 * iproc.setColor(Color.cyan);
		 * 
		 * iproc.setLineWidth(cLineWidth); //iproc.drawLine((int)x1, (int)y1, (int)x2,
		 * (int)y2); iproc.drawLine(x1, y1, x2, y2);
	...
	public void addAnnotation(int mx, int my, boolean dontRemove) {
		/*
		 * if (iIsMainImgWindow) { iTimeInc = iAceTree.getTimeInc(); iImageTime =
		 * iAceTree.getImageTime(); iPlaneInc = iAceTree.getPlaneInc(); } else {
		 * iTimeInc = 0; iPlaneInc = 0; } double x, y, r; boolean g; Nucleus n =
		 * cNucleiMgr.findClosestNucleus(mx, my, iImagePlane + iPlaneInc, iImageTime +
		 * iTimeInc); if (cNucleiMgr.hasCircle(n, (double)(iImagePlane + iPlaneInc))) {
		 * AnnotInfo ai = new AnnotInfo(n.identity, n.x, n.y); // now, if this one is
		 * not in the vector add it // otherwise remove it boolean itemRemoved = false;
		 * boolean itemAlreadyPresent = false; String test = n.identity; AnnotInfo
		 * aiTest = null; for (int k=0; k < iAnnotsShown.size(); k++) { aiTest
		 * =(AnnotInfo)iAnnotsShown.elementAt(k); if (aiTest.iName.equals(test)) {
		 * itemAlreadyPresent = true; if (!dontRemove) { iAnnotsShown.remove(k);
		 * itemRemoved = true; } break; }
		 * 
		 * }
		 * 
		 * if (!itemRemoved && !itemAlreadyPresent) { iAnnotsShown.add(ai); } }
		 */
