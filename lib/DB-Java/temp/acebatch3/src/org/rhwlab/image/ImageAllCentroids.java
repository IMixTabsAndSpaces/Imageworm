...
public class ImageAllCentroids extends JPanel implements ActionListener {
	...
	public void drawNuclei(Graphics2D g, int time) {
		if (time == 1) {
			if (iMenuItem.equals(ALL))
				drawAllNuclei(g, time);
			return;
		}
		...
		for (int i = 0; i < nuclei.size(); i++) {
			...
			// else if (iMenuItem.equals(ATENDTIME) && isAtEndTime(n.identity, time))
			// drawIt(g, n);
		}
	}
	...
	private static final String[] sulstonNames = { "ABaraaaa", "ABalpaaa", "ABalpap", "ABaraapa", "ABarapaa",
			"ABaraapp", "ABarapap", "ABalpappa", "Capa", "Cppa", "Capp", "Cppp", "MSaaaa", "MSpaaa", "MSaaap", "MSaapa",
			"MSpaap", "MSpapa", "MSaapp", "Ealp", "Earp", "Epla", "Epra", "Eplp", "Eprp", "Da", "Dp", "Z"
	...
	/*
	 * private JMenuBar createMenuBar() { JMenuBar menuBar = new JMenuBar(); JMenu
	 * menu = new JMenu("Show"); JMenuItem test = new JMenuItem("All");
	 * menu.add(test); test.addActionListener(this); JMenuItem test2 = new
	 * JMenuItem("Dividing only"); menu.add(test2); test2.addActionListener(this);
	 * JMenuItem test3 = new JMenuItem("Nondividing only"); menu.add(test3);
	 * test3.addActionListener(this); menuBar.add(menu); return menuBar; }
	 */
	...
	private static final String ALL = "All", DIVIDING = "Dividing only", NONDIVIDING = "Nondividing",
			ATENDTIME = "At end time", STANDARD = "Standard view", INVERT_Y = "Invert y axis",
