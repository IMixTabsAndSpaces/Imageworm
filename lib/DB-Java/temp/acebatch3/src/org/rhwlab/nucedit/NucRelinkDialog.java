...
public class NucRelinkDialog extends JDialog implements ActionListener {
	...
	public NucRelinkDialog(AceTree aceTree, Frame owner, boolean modal, Cell cell, int time) {
		...
		// iDLog.append("NucRelinkDialog: " + f.getName() + CS + f.getStyle() + CS +
		// f.getSize());
	...
	void setDefaultButtonBehavior(JButton jb, String key) {
		...
		// jb.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke(s),
		// "pressed");
		jb.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke(KeyEvent.VK_END, 0, false),
				"pressed");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (cmd.equals(SETEARLYCELL)) {
			...
			if (!b)
				return;
			/*
			 * boolean endGood = NucUtils.isValidCell(endCellName, endTime); boolean strGood
			 * = NucUtils.isValidCell(strCellName, strTime); if (!(endGood && strGood)) {
			 * String s0 = ""; String s1 = ""; s0 = "invalid cell: "; if (!endGood) s0 =
			 * "invalid cell: " + endCellName + CS + endTime; if (!strGood) s1 =
			 * "invalid cell: " + strCellName + CS + strTime; JOptionPane pane = new
			 * JOptionPane(s0 + "\n" + s1); JDialog dialog = pane.createDialog(iAceTree,
			 * "About AceTree"); dialog.setModal(true); dialog.show(); //dispose(); return;
			 * }
			 */
	...
	public final static String TITLE = "Nuclei Editor", RELINKTIME = "Later time", RELINKNUC = "Cell name",
			LINKTIME = "Earlier time", LINKNUC = "Cell name", DOIT = "Apply", CS = ", ", NL = "\n",
			APPLYANDREBUILD = "apply/rebuild", APPLYONLY = "apply only", SETCURRENTCELL = "set current cell",
			SETEARLYCELL = "set early cell", SETLATECELL = "set late cell", SETROOTCELL = "set root cell",
			LATER = "link cell at later time", EARLIER = "to cell at earlier time", FIVE = "     ",
			TWELVE = "            ";
