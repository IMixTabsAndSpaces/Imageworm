...
public class SkipFalseNegatives extends JDialog implements ActionListener {
	...
	public SkipFalseNegatives() {
		...
		/*
		 * s = new JPanel(); s.setLayout(new FlowLayout()); iDoit = new JButton(DOIT);
		 * iDoit.addActionListener(this); s.add(iDoit); p.add(s);
		 */
	...
	private final static String TITLE = "Skip False Negatives", RELINKTIME = "Later time", RELINKNUC = "Cell name",
			...
			SKIPFALSENEGS = "skip false negatives", CONTINUESKIPPING = "continue skipping",
			LATER = "cell at earlier time", EARLIER = "search for ancesters of";;
