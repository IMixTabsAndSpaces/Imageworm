...
public class AddOneDialog extends JDialog implements ActionListener, WindowFocusListener {
	...
	public void processMouseEvent(MouseEvent e) {
		...
		if (button == 1) {
			...
			updateCurrentInfo(false);
			Nucleus n = ImageWindow.cNucleiMgr.findClosestNucleus(e.getX(), e.getY(), iImagePlane + iPlaneInc,
					iImageTime + iTimeInc);
			if (n == null) {
...
//        iAceTree.setCurrentCell(c, iImageTime + iTimeInc, AceTree.RIGHTCLICKONEDITIMAGE);
	...
	protected void updateCurrentInfo(boolean detectChange) {
		// System.out.println("EditImage.updateCurrentInfo called: " + new
	...
	public static final String UP = "UP", DOWN = "DOWN", LEFT = "LEFT", RIGHT = "RIGHT", TEST = "TEST", UNDO = "UNDO",
			BIG = "BIG", SMALL = "SMALL", INCZ = "INCZ", DECZ = "DECZ", ADJUST = "adjustCell", ADD = "addCell",
			ADDSERIES = "addCellSeries", REBUILDANDRENAME = "rebuildAndRename", RELINK = "relink",
			KILLCELLS = "killCells", REBUILDONLY = "rebuildOnly", NAME = "Name", FORCENAME = "ForceName", X = "X",
			Y = "Y", Z = "Z", D = "D", FORCE = "Force", SETN = "SetN", SETX = "SetX", SETY = "SetY", SETZ = "SetZ",
			SETD = "SetD"
