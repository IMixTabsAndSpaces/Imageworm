...
public class Lazarus extends JDialog implements ActionListener, ListSelectionListener, Comparator {
	...
	public Lazarus() {
		 ...
		 * BoxLayout.Y_AXIS)); iFont = new Font("roman", Font.PLAIN, 16); iToolBar = new
		 * JToolBar(""); iToolBar.setLayout(new GridLayout(1,0)); add(iToolBar);
		 * buildOutToolBar(); buildList(); initialize(); iTitle = "Lasarus"; iFrame =
		 * new JFrame(iTitle); showMe();
		 */
	...
	private void makeInitialList() {
		...
		while (cells.hasMoreElements()) {
			...
			println("" + laz);
			if (laz.iMatch)
				iListModel.addElement(laz.toString());
		}
		// *****/
	...
	/*
	 * private int getJumpX(int k, Nucleus n) { Laz laz = new Laz(n, k); Vector
	 * nuclei = (Vector)nuclei_record.get(k); for (int i=0; i < nuclei.size(); i++)
	 * { Nucleus np = (Nucleus)nuclei.get(i); if (np.identity.equals(n.identity)) {
	 * return laz.distance(np); } } return 0; }
	 */
