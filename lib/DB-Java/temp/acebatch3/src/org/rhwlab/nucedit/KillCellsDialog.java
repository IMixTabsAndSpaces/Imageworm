...
public class KillCellsDialog extends GenericDialog {
	...
	public KillCellsDialog(AceTree aceTree, Frame owner, boolean modal, Cell cell, int time, EditLog editLog) {
		...
		/*
		 * choices.setLayout(new GridLayout(0,1)); iOneCell = new JRadioButton(ONECELL);
		 * iOneCell.setSelected(true); iOneCell.addActionListener(this); iSeveralCells =
		 * new JRadioButton(SEVERAL); iSeveralCells.addActionListener(this); ButtonGroup
		 * group = new ButtonGroup(); group.add(iOneCell); group.add(iSeveralCells);
		 * choices.add(iOneCell); choices.add(iSeveralCells);
		 */
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (k == 1)
			...
			if (k == 2)
				iEditLog.append("time." + iNucleiMgr.getIndex(iCellName, iTime + 1));
			else {
				int km = k - 1;
				String s = km + " times ";
				for (int i = 0; i < km; i++) {
		...
		for (int i = 0; i < k; i++) {
			...
			/*
			 * if (namingMethod == Identity.MANUAL) { int lastp = indices.size(); for (int j
			 * = lastp; j > 0; j--) { int kn = ((Integer)indices.elementAt(j -
			 * 1)).intValue(); nuclei.remove(kn); } }
			 */
		}
		Cell c = null;
		int strTime = iTime - 1;
	...
	private final static int WIDTH = 300, HEIGHT = 300, MAX_KILLS = 999;

	public final static String TITLE = "Kill Cells", ONECELL = "one cell", SEVERAL = "several cells",
			KILLABLE = " are killable", CELLTOKILL = "kill cell", KILLTIME = "start time", NUMBER = "number",
			SPACER = "     ";
