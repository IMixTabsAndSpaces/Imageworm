...
public class Juvenesence extends JDialog implements ActionListener, ListSelectionListener {
	...
	public Juvenesence() {
		 ...
		 * GridLayout(1,0)); add(iToolBar); initialize(); buildOutToolBar();
		 * buildList(); iFrame = new JFrame("Juvenesence"); showMe();
		 */
	...
	public void lifetimesTest(double tolL, double tolH) {
		...
		for (int i = base.size() - 1; i >= 0; i--) {
			...
			if (obj == null)
				continue; // parent in but we dont have its lifetime
	...
	private static double MINRATIO = 0.8, MAXRATIO = 2.5;
