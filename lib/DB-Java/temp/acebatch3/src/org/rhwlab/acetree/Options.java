...
public class Options extends JDialog implements ActionListener {
	...
	static final String[] ZIPCHOICE = { "tif", "zip1", "zip2", "jpg" };
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (x != iCancel) {
			...
			if (m != iLineWidthInUse)
				ImageWindow.cLineWidth = m;
			int namingMethod = Identity.MANUAL;
			if (iStandard.isSelected())
				namingMethod = Identity.STANDARD;
			// else if (iCanonical.isSelected()) namingMethod = Identity.CANONICAL;
			else if (iNewCanonical.isSelected())
				namingMethod = Identity.NEWCANONICAL;
			...
			boolean useAxis = !axis.equals(iAxisInUse) && iNewCanonical.isSelected() && iUseAxis.isSelected();
			if (useAxis)
				iNucleiMgr.getConfig().iAxisGiven = axis;
			// println("Options.actionPerformed: " + axis + CS + useAxis);
			if (namingMethod != iNamingMethod || divisor != iDivisorInUse || minCutoff != iMinCutoffInUse || useAxis) {
	...
	private static final String[] AXES = { "auto", "adl", "avr"

	};

	private static final String TITLE = "Options", STANDARD = "Standard"
	// ,CANONICAL = "Canonical"
			, NEWCANONICAL = "New canonical", MANUAL = "Manual", APPLY = "Apply", CANCEL = "Cancel";
