...
public class AceTreeNoUI {
	...
	private void bringUpSeries(String configFileName) {
		...
		ImageWindowNoUI.setStaticParameters(iNucleiMgr.getConfig().iZipTifFilePath, iNucleiMgr.getConfig().iTifPrefix,
	...
	public int bringUpSeriesData(Config config) {
		...
		/*
		 * String configName = nucMgr.getConfig().getShortName(); if
		 * (!iNucleiMgrHash.containsKey(configName)) { iNucleiMgrHash.put(configName,
		 * nucMgr); iAceMenuBar.addToRecent(configName); }
		 */
		System.gc();
		return 0;
	}
	...
	public static void main(String[] args) {
		...
		int length = v.size();
		/*
		 * for (int i=0; i < v.size(); i++) { CellData cd = (CellData)v.get(i); Nucleus
		 * n = cd.iNucleus; println("main, " + n.identity + CS + n.z + CS + (tstart +
		 * i));
		 * 
		 * }
		 */
		...
		/*
		 * Enumeration e = h.keys(); while (e.hasMoreElements()) { String cname =
		 * (String)e.nextElement(); Cell c = (Cell)h.get(cname); Vector v =
		 * c.getCellData(); println("main, " + cname + CS + c + CS + v.size()); }
		 */
		...
		g.dispose();
		/*
		 * if (showSelectedCell) { Nucleus n = cd.iNucleus; Graphics g =
		 * bi.getGraphics(); g.setColor(Color.WHITE); g.drawString(n.identity, n.x,
		 * n.y); g.dispose();
		 * 
		 * }
		 */
