...
public class Cell extends DefaultMutableTreeNode {
	...
	/**
	 * @param name String name of Cell also stored in the parent as userObject
	 */
	...
	private void drawColoredLine(Graphics g, Cell c, int x1, int y1, int x2, int y2) {
		// System.out.println("\ndrawColoredLine entered: " + iName + CS + c.getName() +
		// CS + x1
		...
		if (x1 != x2) {
			...
			if (k > 0) {
				...
				for (int i = 0; i < k; i++) {
					...
					// System.out.println("drawColoredLine: " + i + CS + k + CS +c.iCellData.size()
					// + CS + c.getName() + CS + useSize + CS + use.elementAt(i));
					g.setColor(color);
	...
	public void draw(Graphics g, int w, int h, int frameWidth, Hashtable cHash) {
		...
		// System.out.println("Cell.draw: " + iTimeIndex + CS + iEndingIndex + CS +
		// iLateTime);
		// height = 800;
	...
	private int draw(Graphics g, int h, int x, int ystart, Cell c, Hashtable cHash, int rootStart) {
		...
		// System.out.println("draw2 " + c.getName() + CS + this.getName() + CS +
		// lastTime + CS + lateTime);
		...
		// System.out.println("draw2: " + c + CS + iTimeIndex + CS + c.iTimeIndex + CS +
		// c.iEndTime);
		...
		if (c.getChildCount() == 0 || done) {
			...
			// System.out.println("draw1: " + iName + CS + c.getName() + CS + done + CS +
			// g.getColor());
			g.setColor(Color.black);
			...
			if (!isLeaf(cLeft)) {
				...
				// System.out.println("drawRotated: " + cLeft.getName() + CS + cLeft.isLeaf() +
				// CS + x1 + CS + x);
			}
			int xx = x1 + xsc * nl;
			...
			// System.out.println("draw2: " + iName + CS + c.getName() + CS + done + CS +
			// g.getColor());
			x = (x1 + x2) / 2;
			// g.drawLine(x, c.yStartUse, x, cLeft.yStartUse);
			drawColoredLine(g, c, x, c.yStartUse, x, cLeft.yStartUse);
			// System.out.println("draw3: " + iName + CS + c.getName() + CS + done + CS +
			// g.getColor());
			// if (x1 != x && !cLeft.isLeaf()) {
			// g.fillOval(cLeft.xUse - 4, cLeft.yStartUse - 4, 8, 8);
			// fillInHash(cLeft, cHash);
			// drawRotatedText(g, cLeft.getName(), cLeft.xUse, cLeft.yStartUse - 5,
			// -Math.PI/8);
			// }
			// if (x2 != xx && !cRite.isLeaf()) {
			// g.fillOval(cRite.xUse - 4, cRite.yStartUse - 4, 8, 8);
			// fillInHash(cRite, cHash);
			// drawRotatedText(g, cRite.getName(), cRite.xUse, cRite.yStartUse - 5,
			// -Math.PI/8);
			// }
			return x;
		}
	...
	private void drawRotatedText(Graphics g, String s, int x, int y, double angle) {
		...
		// System.out.println("drawRotatedText1: " + s + CS + p1.getX() + CS + p1.getY()
		// + CS + angle);
		...
		// System.out.println("drawRotatedText2: " + s + CS + p1.getX() + CS +
		// p1.getY());
	...
	public Vector getCellData(int start, int end) {
		// System.out.println("getCellData: " + iName + CS + start + CS + end + CS +
		// iTimeIndex + CS + iEndTime);
		if (start <= iTimeIndex && end > iEndTime)
			return iCellData;
		if (start > iEndTime)
			return null;
		Vector v = new Vector();
	...
	public String getRedDataString(int first, int last, int separator, int[] count) {
		...
		for (i = 0; i < iCellData.size(); i++) {
			...
			if (k == 0)
				s += sep + ONEDEC.format(d + 0.1);
			else
				s += sep + NODEC.format(d);
	...
	public String getReverseRedDataString(int first, int last, int separator, int[] count) {
		// System.out.println("getReverseRedDataString: " + iName + CS + first + CS +
		// last);
		String sep = SEPARATORS[separator];
		int k = 0;
		String s = "";
		...
		for (int i = iCellData.size() - 1; i >= 0; i--) {
			...
			// System.out.println(iName + CS + i +CS + k + CS + (i + iTimeIndex) + CS + last
			// + CS + d);
			k++;
		}
	...
	public void setParameters(int time, int endTime, Nucleus n) {
		// System.out.println("Cell.setParameters: " + iName + CS + time + CS +
		// endTime);
		iTimeIndex = time;
	...
	public void showParameters() {
		/*
		 * System.out.println("showParameters starting"); System.out.println("name: " +
		 * iName); System.out.println("time: " + iTimeIndex);
		 * System.out.println("plane: " + iPlane); System.out.println("x: " + iX);
		 * System.out.println("y: " + iY); System.out.println("dia: " + iDia);
		 * System.out.println("endTime: " + iEndTime); System.out.println("endFate: " +
		 * fates[iEndFate]); System.out.println("hashKey: " + iHashKey);
	...
	private static final Color[] CMAP = { new Color(000, 30, 255), new Color(000, 55, 230), new Color(000, 80, 205),
			new Color(000, 105, 180), new Color(000, 130, 155), new Color(000, 155, 130), new Color(000, 180, 105),
			new Color(000, 205, 80), new Color(000, 230, 55), new Color(000, 255, 30), new Color(30, 230, 000),
			new Color(55, 205, 000), new Color(85, 180, 000), new Color(105, 155, 000), new Color(130, 130, 000),
			new Color(155, 105, 000), new Color(180, 80, 000), new Color(205, 55, 000), new Color(230, 30, 000),
			new Color(255, 000, 000) };

	private static final Color[] CMAP3 = { new Color(000, 000, 255), new Color(000, 051, 255), new Color(000, 103, 255),
			new Color(000, 153, 255), new Color(000, 204, 255), new Color(000, 255, 255), new Color(000, 255, 204),
			new Color(000, 255, 153), new Color(000, 255, 103), new Color(000, 255, 051), new Color(000, 255, 000),
			new Color(051, 255, 000), new Color(153, 255, 000), new Color(204, 255, 000), new Color(255, 255, 000),
			new Color(255, 204, 000), new Color(255, 153, 000), new Color(255, 102, 000), new Color(255, 051, 000),
			new Color(255, 000, 000) };

	private static final Color[] CMAP2 = { new Color(000, 255, 0), new Color(000, 230, 0), new Color(000, 205, 0),
			new Color(000, 180, 0), new Color(000, 155, 0), new Color(000, 130, 0), new Color(000, 105, 0),
			new Color(000, 80, 0), new Color(000, 55, 0), new Color(000, 30, 0), new Color(30, 000, 0),
			new Color(55, 000, 0), new Color(85, 000, 0), new Color(105, 000, 0), new Color(130, 000, 0),
			new Color(155, 000, 0), new Color(180, 000, 0), new Color(205, 000, 0), new Color(230, 000, 0),
			new Color(255, 000, 0) };

	public final static int NAME = 4, TIME = 0, PLANE = 3, X = 1, Y = 2, DIA = 5, PREV = 12 // index for this cell in
																							// the previous nuclei file
	...
	public final static int ALIVE = 0, DIVIDED = 1, DIED = 2;

	public final static int LARGEENDTIME = 500;

	public final static String[] fates = { "alive", "divided", "died" };
