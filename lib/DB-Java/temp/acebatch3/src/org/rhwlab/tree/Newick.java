...
/**
 * Creates a representation of the tree as a string in the Newick file format
 * and passes the string to the ATV program for rendering as a Sulston style
 * tree
 * 
 * @author biowolp
...
public class Newick extends JPanel {
	 ...
	 * 
	 * @param root a cell treated as the root of a tree -- typically "start" Our
	 *             trees may have several "roots" arising from the fact that the
	 *             processing typically starts at the 4 cell stage. We treat the 4
	 *             founder cells as separate trees.
	 */
	...
	/**
	 * Create a vector containing the string parts of a newick representation of the
	 * tree rooted on Cell x
	 * 
	 * @param x the cell at the root of this "subtree"
	 * @return a vector of strings that are pieces of the newick subtree for x
	 */
	...
	/**
	 * Debugging function to show the Vector of strings created in processTree()
	 * 
	...
	/**
	 * recursive function to do a traversal of the tree not sure about pre or post
	 * order -- this version was tinkered with to work on the 4to100Cells data where
	 * working means that the order of branches was well aligned with the Sulston
	 * tree when called from the AceTree tree of that data
	 * 
	...
	/**
	 * return either s or a "null" string depending on whether the name s is in a
	 ...
	 */
	/*
	 * private void showATVtree(String s) { Tree tree = null; try { tree = new
	 * Tree(s); ATVjframe atvframe = new ATVjframe( tree ); atvframe.showWhole(); }
	 * catch ( Exception e ) { System.out.println( "ATVapp not available: " + e );
	 * //System.exit( -1 ); }
	 * 
	 * //ATVjframe atvframe = new ATVjframe( tree ); //atvframe.showWhole();
	 * 
	 * }
	 */
	...
	/**
	 * The set of interior cells that are to be labeled in the newick
	 * representation. Other interior cells are only tagged with their lifetimes.
	 */
	private static final String[] blastomeres = { "P0", "P1", "AB", "P2", "EMS", "ABa", "ABp", "MS", "E", "P3", "C",
			"P4", "D" };
