...
/**
 * a class to emulate the Nucleus_t struct in StarryNight <br>
 * all member variables have public access; this makes comparisons with the C
 * code, where needed, easy
 * 
 * @author biowolp
 * @version 1.0
 */
public class Nucleus implements java.util.Comparator {
	...
	/**
	 * default constructor needed when the editing process calls for a new nucleus
	 * -- it then fills in the data <br>
	 * There will be one such object for each line in each nuclei file
	 * 
	 */
	public Nucleus() {
		super();
	...
	/**
	 * constructor used when nuclei are read in from files using the new file format
	 * 
	 * @param sa String [] with parsed entries from the line in the file
	 */
	public Nucleus(String[] sa) {
		...
		if (sa[PRED].equals(NILL) || Integer.parseInt(sa[PRED]) == -1)
			predecessor = NILLI;
		else
			predecessor = Integer.parseInt(sa[PRED]);
		if (sa[SUCC1].equals(NILL))
			successor1 = NILLI;
		else
			successor1 = Integer.parseInt(sa[SUCC1]);
		if (sa[SUCC2] == null)
			successor2 = NILLI;
		else
			successor2 = Integer.parseInt(sa[SUCC2]);
	...
	/**
	 * constructor used when nuclei are read in from files using the old file format
	 * 
	 * @param sa String [] with parsed entries from the line in the file
	 */
	public Nucleus(String[] sa, boolean oldFormat) {
		...
		if (sa[OPRED].equals(NILL) || Integer.parseInt(sa[OPRED]) == -1)
			predecessor = NILLI;
		else
			predecessor = Integer.parseInt(sa[OPRED]);
		if (sa[OSUCC1].equals(NILL))
			successor1 = NILLI;
		else
			successor1 = Integer.parseInt(sa[OSUCC1]);
		if (sa[OSUCC2] == null)
			successor2 = NILLI;
		else
			successor2 = Integer.parseInt(sa[OSUCC2]);
	}
	...
	private int getRedChoiceNumber(String type) {
		...
		for (i = 0; i < Config.REDCHOICE.length; i++) {
			if (type.equals(Config.REDCHOICE[i]))
				break;
		}
	...
	final public static int INDEX = 0, X = 5, Y = 6, Z = 7, IDENTITY = 9, SIZE = 8, WT = 10, STATUS = 1, PRED = 2,
			SUCC1 = 3, SUCC2 = 4, RWT = 11, RSUM = 12, RCOUNT = 13, ASSIGNEDID = 14, RWRAW = 15, RWCORR1 = 16,
			RWCORR2 = 17, RWCORR3 = 18, RWCORR4 = 19;

	// * the locations of things in the old file format
	final public static int OINDEX = 0, OX = 1, OY = 2, OZ = 3, OIDENTITY = 4, OSIZE = 5, OWT = 6, OSTATUS = 7,
			OSPONG1 = 8, OSPONG2 = 9, OPIECE1 = 10, OPIECE2 = 11, OPRED = 12, OSUCC1 = 13, OSUCC2 = 14;
	// */
	final private static String NILL = "nill", SPACE14 = "              ";

	final public static int NILLI = -1;
