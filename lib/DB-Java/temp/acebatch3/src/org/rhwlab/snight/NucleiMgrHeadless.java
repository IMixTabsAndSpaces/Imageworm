...
public class NucleiMgrHeadless extends NucleiMgr {
	...
	/*
	 * private AceTree iAceTree; private ZipNuclei iZipNuclei; File iFile;
	 * FileOutputStream iFOS; PrintWriter iPWriter; Vector nuclei_record; Vector
	 * nuclei_record_backup; Parameters Parametersx; String iParameterEntry; int
	 * iStartingIndex; int iEndingIndex; int iImageWidth; int iImageHeight; int
	 * iIndexOffset; int iNumNucleiFiles; String [] parameterFileData; boolean
	 * iFakeNuclei; private boolean iDebug;
	 * 
	 * //private Identity iIdentity; private Identity3 iIdentity;
	 * 
	 * private AncesTree iAncesTree; private Config iConfig; private MeasureCSV
	 * iMeasureCSV; private Parameters iParameters; private Movie iMovie; private
	 * EditLog iEditLog; private Log iDDLog; private Log iDLog; private boolean
	 * iEditLogInitialized; private double iZPixRes;
	 * 
	 * 
	 * private int iPlaneEnd; private int iPlaneStart; private int iLastNucleiFile;
	 * 
	 * public boolean iGoodNucleiMgr;
	 * 
	 * PrintWriter iPrintWriter;
	 * 
	 * private static String p2 = "t"; private static String p3 = "-nuclei";
	 */
	...
	public void readNuclei() {
		...
		// System.out.println("last, iEndingIndex: " + last + CS + iEndingIndex + CS +
		// iConfig.iEndingIndex + CS + iMovie);
	}
	...
	private int readNuclei(ZipNuclei zn) {
		...
		// System.out.println("readNuclei:1 " + iMovie.time_end + CS +
		// iMovie.time_start);
		fakeNuclei();
		...
		// System.out.println("readNuclei:2 " + iMovie.time_end + CS +
		// iMovie.time_start);
		while (e.hasMoreElements()) {
			...
			if (saa[0].equals("nuclei")) {
				...
				// println("ZipNuclei.main: finds: " + saa[1] + CS + ze.isDirectory() + CS +
				// index);
				...
				while (s != null && s.length() > 10) {
					...
					try {
						...
						if (newFormat)
							...
							sa = getTokens(s);
						if (sa[0] != null && (sa[0].length() > 0 && Integer.parseInt(sa[0]) != j++)) {
							break;
						}
						if (newFormat)
							n = new Nucleus(sa);
		...
		// println("readNuclei: iEndingIndex=" + iEndingIndex + CS + iLastNucleiFile +
		// CS + nuclei_record.size());
		if (iEndingIndex == 1) {
		...
		// System.out.println("readNuclei:3 " + iMovie.time_end + CS +
		// iMovie.time_start);
		return 999;
	...
	public void sendStaticParametersToImageWindow() {
		ImageWindow.setStaticParameters(iConfig.iZipTifFilePath, iConfig.iTifPrefix, iConfig.iUseZip);
	}
	...
	public Nucleus findClosestNucleus(int mx, int my, int time) {
		...
		// System.out.println("returning candidate: " + candidate.identity + CS +
		// candidate.z);
		return candidate;
	}

	public Nucleus findClosestNucleus(int mx, int my, int mz, int time) {
		// System.out.println("findClosestNucleus: " + mx + CS + my + CS + mz + CS +
		// time);
		if (time < 1)
			time = 1;
		Vector nuclei = null;
	...
	private void getScopeParameters() {
		...
		// System.out.println("getScopeParameters: z_res_fudge=" +
		// iParameters.z_res_fudge);
	...
	/**
	 * called from AceTree when no nuclei file was specified we will add either
	 * iEndingIndex or LAST empty nuclei vectors LAST if iEndingIndex = 1 meaning no
	 * ending index was specified if LAST then extras are removed after all nuclei
	...
	public Parameters fakeParameters(String tifPath, String tifPrefix) {
		...
		createDummies(p);
		/*
		 * System.out.println("fakeParameters: " + tifPath + CS + tifPrefix); String s =
		 * iAceTree.makeImageName(1,15); System.out.println("fakeParameters2: " + s);
		 * String name = tifPath + File.separator + tifPrefix + s; File f = new
		 * File(name); System.out.println("fakeParameters3: " + f);
	...
	/**
	 * major access function to the array of nuclei Vectors stored here -- one
	...
	public void setAllSuccessors() {
		// System.out.println("setAllSuccessors: " + iStartingIndex + CS + iEndingIndex
		// + ", " + nuclei_record.size());
	...
	public int setSuccessors(int i) {
		...
		// }
		if (iConfig.iNamingMethod == Identity.MANUAL)
			return 0;
		...
		for (int j = 0; j < next.size(); j++) {
			...
			if (n.status == Identity.DEAD)
				continue;
			int pred = n.predecessor;
			if (pred == Identity.DEAD)
				continue;
	 ...
	 *
	 * a line in such a file consists of 15 fields which were written by a printf
	 * statement in a C program The widths of the fields are specified in the static
	 * int [] X 1 index -- line number in file starting from 1 2 x location of
	 * nucleus in pixels 3 y location of nucleus in pixels 4 z location of nucleus
	 * in planes (float) 5 cell name assigned by StarryNight 6 nominal cell diameter
	 * in pixels (all cells in a time index have the same value) 7 measure of GFP
	 * signal within the circle of this nucleus 8 ? (seems to be a flag) 9 ? (float)
	 * 10 ? (float) 11 ? (float) 12 ? (float) 13 index - the index that this cell
	 * had in the previous time nuclei file 14 index - the index that this cell has
	 * in the next time nuclei file 15 index - (if present indicates a division
	 * occurred) index of second daughter in next time nuclei file
	 *
	 *
	 *
	 ...
	 */
	private static final int[] XX = { 6, 5, 5, 6, 14, 4, 8, 4, 5, 6, 5, 6, 5, 5, 5 };

	public static final int LAST = 599 // more than the most nuclei we ever expect to see
	;

	final public static String PARAMETERS = "parameters", NL = "\n", CS = ", ";
