...
/**
 * Contains a UI and three active functions to report on a lineage: Overview
 * Identity check Fate check
 *
 * @author biowolp
...
public class Analysis extends Log {
	...
	public Analysis(AceTree aceTree, String title) {
		...
		addToolBarButton(jb);
		/*
		 * jb = new JButton(TEST1); addToolBarButton(jb); jb = new JButton(TEST2);
		 * addToolBarButton(jb); jb = new JButton(TEST3); addToolBarButton(jb); jb = new
		 * JButton(TEST4); addToolBarButton(jb); jb = new JButton(TEST5);
		 * addToolBarButton(jb); jb = new JButton(TEST6); addToolBarButton(jb);
		 */
		initialize();
		...
		for (int j = 0; j < nuclei.size(); j++) {
			...
			if (n.status >= 0) {
				...
				if (time == nuclei_record.size()) {
					...
					if (c.getFateInt() != Cell.ALIVE)
						append(c + CS + c.getFate());
				}
			}
		}
		return count;
	...
	private void showLeaves() {
		...
		do {
			i++;
			fate = c.getFateInt();
			if (fate == Cell.ALIVE || fate == Cell.DIVIDED)
				alive++;
			else if (fate == Cell.DIED)
				deaths++;
	...
	private void test3() {
		...
		/*
		 * Enumeration e2 = ourCells.keys(); while (e2.hasMoreElements()) {
	 ...
	 * private void test6() { Hashtable relPosHash =
	 * iNucleiMgr.getIdentity().getRelativePositionHash(); Enumeration e =
	 * relPosHash.keys(); append("relPosHash has " + relPosHash.size() +
	 * " entries."); Vector ones = new Vector(); Vector twos = new Vector(); Vector
	 * threes = new Vector(); while(e.hasMoreElements()) { String s =
	 * (String)e.nextElement(); Integer kI = (Integer)relPosHash.get(s); int k =
	 * kI.intValue(); switch(k) { case 1: case -1: ones.add(s); break; case 2: case
	 * -2: twos.add(s); break; case 3: default: threes.add(s); } //if (Math.abs(k)
	 * != 1) append(s + C.CS + k); } append(ones.size() +
	 * " cells named by x axis relative position."); append(twos.size() +
	 * " cells named by y axis relative position"); e = twos.elements(); while
	 * (e.hasMoreElements()) append((String)e.nextElement()); append(threes.size() +
	 * " cells named by z axis relative position"); e = threes.elements(); while
	 * (e.hasMoreElements()) append((String)e.nextElement()); }
	 */
	...
	private static final String TEST1 = "Test1", TEST2 = "Test2", TEST3 = "Test3", TEST4 = "Test4", TEST5 = "Test5",
			TEST6 = "Test6", CLEAR = "Clear", SAVEAS = "Save as", OVERVIEW = "Overview",
			IDENTITYCHECK = "IdentityCheck", FATECHECK = "FateCheck", DIVISIONCHECK = "DivisionCheck",
			ACTIONS = "Actions", FILE = "File", CS = ", ", NL = "\n";
