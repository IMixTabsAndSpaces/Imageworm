...
public class Analysis3 extends Log {
	...
	/*
	 * private void test22() { append("test2 entered: "); initialize(); Enumeration
	 * e = iNucleiMgrHash.keys(); while(e.hasMoreElements()) { String config =
	 * (String)e.nextElement(); NucleiMgr nucMgr =
	 * (NucleiMgr)iNucleiMgrHash.get(config); int ap =
	 * nucMgr.getParameters().apInit; int dv = nucMgr.getParameters().dvInit; int lr
	 * = nucMgr.getParameters().lrInit; append(config + C.CS +
	 * nucMgr.getImageWidth() + C.CS + nucMgr.getImageHeight()+ C.CS + ap + C.CS +
	 * dv + C.CS + lr + C.CS + ap*dv); //append(config + C.CS +
	 * nucMgr.getImageWidth() + C.CS + nucMgr.getImageHeight()); } }
	 */
	...
	/**
	 * Here I want to examine a time point and develop for each valid nucleus a
	 * structure giving a normalized x, y, z, size, and identity which in the long
	 * run will be used to classify the cell using some kind of crazy classifier
	 *
	 */
	...
	private void computeStats(Vector nnv, int[] ia) {
		...
		// Standard Deviation = square root of[ (sum of Xsquared -((sum of X)*(sum of
		// X)/N))/ (N-1)) ]
	...
	private static final int XMM = 350, YMM = 250, ZMM = 180, XSDD = 130, YSDD = 80, ZSDD = 60, SIZESCALE = 5;
	...
	private static final String CLEAR = "Clear", LINE = "                                        ", ANGLE = "Angle",
			TEST1 = "Test1", TEST2 = "Test2", TEST3 = "Test3", TEST4 = "Test4", TEST5 = "Test5", EXIT = "Exit",
			CONFIGS = "ConfigFiles";
