...
public class RedBkgComp2 extends Log {
	...
	public void setParameters(int start, int end, double kMedium, double kLarge, RedBkgComp redBkgComp,
	...
	public void processHashPlanes(int time, Vector hashPlanes) {
		...
		for (int j = 0; j < nuclei.size(); j++) {
			...
			// println("processHashPlanes, " + fri + CS + bri + CS + time + CS +
			// n.identity);
			if (iFeedbackRequested) {
		...
		/*
		 * Collections.sort(iKeys); for (int i=0; i < iKeys.size(); i++) { Totals t =
		 * (Totals)totals.get(iKeys.get(i));
		 * 
	...
	/*
	 * this function processes the image associated with the plane represented by
	 * the Centroids in hashtable h three representations of that image are used
	 * ipData deals with the nuclear red information ipCopy gets middle circles
	 * zeroed out so it sort of has only background information ipTemplate is a
	 * dummy page whose size corresponds to that of ipData. It is used to estimate
	 * the usable area of the annuli in which background red will be estimated
	 *
	 * the first loop is used to zero out the middle sized circles in ipCopy and
	 * ipTemplate
	 *
	 * the second loop adds to the Centroid object the information about total red
	 * signal and "area" of the two regions: nucleus and annulus
	 */
	...
	/*
	 * this function takes a Centroid representing one cell and sets the red and
	 * area data into it this is the hard work of the whole process we examine a
	 * region corresponding to the large circle and within it we locate background
