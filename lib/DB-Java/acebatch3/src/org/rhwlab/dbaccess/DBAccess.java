...
public class DBAccess {
	...
	/*
	 * public static Embryo getEmbryo(String seriesName) { Embryo embryo = null;
	 * File f = new File(cDBLocation); File [] files = f.listFiles(); for (int i=0;
	 * i < files.length; i++) { String candidateName = files[i].getName();
	 * //println("getEmbryo, " + candidateName); if
	 * (candidateName.equals(seriesName)) { String absPath =
	 * files[i].getAbsolutePath(); EmbryoXML exml = new EmbryoXML(absPath, 0);
