...
/**
 * Creates the abstract tree structure from the analysis "nuclei" files. <br>
 * The contents of the files have been read and ensconced in the iNucleiMgr
 * object <br>
 * Relies on Cell to represent the tree. Cell is derived from java class
 * DefaultMutableTreeNode
 * <p>
 * Constructed in <code>AceTree.buildTree</code>. <br>
 * Construction implies processing data stored in a NucleiMgr object and
 * building the tree where the root node is <code>Cell iRoot</code>, which is
 * essentially a complete representation of the embryo tree as seen in the data.
 * Access functions reach essential information developed when the instance is
 * constructed.
 * <p>
 * <br>
 * <code>getRoot</code>returns <code>iRoot</code> <br>
 * <code>getCells</code>returns <code>Hashtable iCells</code> which enables any
 * Cell in the tree to be accessed by its name.
 * <p>
 * <p>
 *
 *
...
public class AncesTree implements Comparator {
	...
	/**
	 * loops through -nuclei file data after it has been processed into the
	 * NucleiMgr object
	 */
	private void processEntries() {
		// System.out.println("starting and ending indices: " + iStartingIndex + ", " +
		// iEndingIndex);
		int count = 0;
		...
		// System.out.println("total cells alive at end: " +
		// countAliveCellsAtIndex(iEndingIndex));
	}
	...
	/**
	 * creates and updates Cell objects found in the current "file" gets the
	 * "nuclei" for this time frame from NucleiMgr this is a vector of nucleus
	 * things more or less mimicing the StarryNight nucleus structure
	 *
		...
		if (index == iStartingIndex) {
			...
			for (int j = 0; j < nuclei.size(); j++) {
				...
				if (prevn.successor2 == Nucleus.NILLI) {
					...
					// println("processEntry, adding to iCells, " + daughter.getHashKey() + CS +
					// daughter.getName());
	...
	private void makeCellsByNameHash() {
		...
		while (e.hasMoreElements()) {
			...
			// System.out.println("makeCellsByName: " + hashKey + CS + key + CS +
			// c.showStuff());
		}
		/*
		 * Enumeration e = iCells.elements(); while (e.hasMoreElements()) { Cell c =
		 * (Cell)e.nextElement(); key = c.getName(); if (key.equals(POLAR) && !b) { key
		 * = POLAR + ++pct; } iCellsByName.put(key, c);
	...
	/**
	 * make sure daughter cells are ordered in the Sulston standard way so the first
	 * daughter is an a, l, d do not handle cells without such ending characters
	 * 
	...
	private void processRootCell(int i, Nucleus n) {
		...
		// System.out.println("processRootCell4: " + iCells.size() + CS +
		// c.showStuff());
		// iCells.put(n.identity, c); //#############################
		// System.out.println("processRootCell putting: " + c.getHashKey() + ", " +
		// c.getName());
		// c.showParameters();
	...
	/**
	 * access function for a hash permitting Cells to be accessed by name
	 * 
	...
	private static final String[] fgo = { "P0", "AB", "P1", "ABa", "ABp", "EMS", "P2" };

	private static final String POLAR = "polar", CS = ", ",
			DIVISIONTEMPLATE = "  Division:                 ->                ",
			DEATHTEMPLATE = "  Death:                ";

	private static final int DEAD = -1, PARLOC = 12, DAULOC = 31, DEATHLOC = 9;
