 ...
 *
 * Class to be run as a separate thread by Image3D2.java to create overlay 
...
public class Image3DOverlayGenerator extends Thread {
    ...
    // An Image3DOverlayGenerator object is first created in Image3D2.java, where it is started immediately.
    // It will start by loading the first possible overlay and then will wait. The thread will be interrupted 
    // when Image3D2 refreshes the display and an overlay reload is needed, so here it
    // will catch the interrupt, reset the needreload flag and continue to reload.
    public void run() {
        while(true) {
            try {
                ...
                // wait until interrupted next ie by Image3D2 while refreshing display
                synchronized(this) {
                    this.wait();
                }
    ...
    public void loadImages() throws InterruptedException {
            ...
            */
            ...
            for(int i = 0; i < getSliceCount() && !Thread.currentThread().isInterrupted(); i++) {
                // for each of the 30 slices available for this timepoint, add -p# to the end of the title
    ...
    */
     ...
     * Generate a Shape3D object that draws spheres for each of the pixels pulled from each of the 
         * 30 2D image slices of the embryo
         *
         * @param pointSize
        ...
        public Shape3D generateOverlay(float pointSize) throws InterruptedException { 
            if(this.coordinates.size() > 0) {
                PointArray p = new PointArray(this.coordinates.size(), PointArray.COORDINATES | PointArray.COLOR_4);
                ...
                for (int x = 0; x < img.getWidth(); x++) {
                    ...
                    for (int y = 0; y < img.getHeight(); y++) {
                        ...
                        if(x%subSample==0 && y%subSample==0){  
                            ...
                            // add to coordinates
                            xf = (float) ((x-geoMgr.iXA)/scale);
                            yf = (float) ((y-geoMgr.iYA)/scale);
    ...
    /**
     * Load the 30 buffered image slices into memory, translate them into a set of points that can be
    ...
    public float calculateOverlayPointSize() {
        return (float) viewConfig.getOverlaySubsample() * 5f;
    }
}
