       ...
       implements ActionListener
{
	...
	return ((int)Math.round(position * 100./ (double)m_imagePanel.getZoomedTo()));
    }
    ...
    void getPositionData(MouseEvent e) {
    	int x = e.getX();
    	int y = e.getY();
    	int x2 = (int)Math.round(x * 100./ (double)m_imagePanel.getZoomedTo());
    	int y2 = (int)Math.round(y * 100./ (double)m_imagePanel.getZoomedTo());
