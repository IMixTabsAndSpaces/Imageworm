...
public class Cell extends DefaultMutableTreeNode {
	...
	//if(iName.equals("P")){//identity==ROOTNAME){
	//    return draw2( g, h,  x,  ystart,  c,  cHash,  rootStart);
	//	}
    ...
    //abortive attempt to make it possible to draw all nuc subtrees if you type in P (ROOTNAME)
    /*
    private int draw2(Graphics g, int h, int x, int ystart, Cell c, Hashtable cHash, int rootStart) {
	    ...
	    int xval=x;
	    //for (int cind=0;cind<children.size();cind++){
	    Iterator it=children.iterator();
	    while (it.hasNext())
	    {	
		Cell cnext = (Cell)it.next(); //c.getChildAt(cind);
		int nl = cnext.getChildCount()/2;
		if (nl == 0) nl = 1;
		// int nr = cRite.getChildCount() + 1;
		int x1 = draw(g, h, xval, yStartUse + length, cnext, cHash, rootStart);
		cnext.xUse = x1;
		if (!isLeaf(cnext)) {
		    g.fillOval(cnext.xUse - 2, cnext.yStartUse - 2, 4, 4);
		    fillInHash(cnext, cHash);
		    drawRotatedText(g, cnext.getName(), cnext.xUse, cnext.yStartUse - 5, -Math.PI/8);
		}
		xval = xval+ x1 + xsc * nl;
	    }
	    return xval;
